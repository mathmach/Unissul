...
<html lang="pt-BR">
    ...
    <body>  
        ...
        <div class="container">
            <c:forEach items="${rackList}" var="rack" varStatus="loop">
                <c:if test="${loop.count mod 2 ne 0}"><div class="row"></c:if>
                    ...
                    <c:if test="${loop.count mod 2 eq 0}"></div></c:if>                        
                </c:forEach>    
                <c:if test="${rackList.size() mod 2 eq 0}"><div class="row"></c:if>
                    <div id="rack" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                        <div class="panel-group">
                            <a id="newRack" href="#">
                                <div class="panel panel-default">
                                    <div class="panel-body">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
                                                <span><strong>Adicionar</strong> nova prateleira</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <c:if test="${rackList.size() mod 2 ne 0}"></div></c:if>
            </div>  
            <!--<div class="container">
                <div class="row">
        ...
        <c:if test="${empty param.pagina}">
            <li>
                <a class="btn disabled" href="" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
        </c:if>                    
        <c:if test="${not empty param.pagina}">                        
            <c:if test="${param.pagina eq 1}">
                <li>
                    <a class="btn disabled" href="" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </a>
            ...
            <c:if test="${not (param.pagina eq 1)}">
                <li>
                    <a class="btn" href="<c:url value="/?pagina=${param.pagina - 1}" />" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                </li>
            </c:if>
        </c:if>
        <c:forEach begin="${pageStart}" end="${pages + 1}" varStatus="loop">
            <c:if test="${empty param.pagina}">
                <c:if test="${loop.count eq 1}">
                    <li class="active">
                        <a href="<c:url value="/" />">${loop.current}</a>
                    </li>  
                </c:if>                                
                <c:if test="${loop.current ne 1}">
                    <li>
                        <a href="<c:url value="/?pagina=${loop.current}" />">${loop.current}</a>
                    </li>  
                </c:if>
            </c:if>                            
            <c:if test="${not empty param.pagina}">          
                <c:if test="${param.pagina eq loop.current}">
                    <li class="active">
                        <a href="<c:url value="/?pagina=${loop.current}" />">${loop.current}</a>
                    </li> 
                </c:if>
                <c:if test="${param.pagina ne loop.current}">
                    <li>
                        <a href="<c:url value="/?pagina=${loop.current}" />">${loop.current}</a>
                    </li> 
                </c:if>
        ...
        <c:if test="${empty param.pagina}">
            <c:if test="${1 < pages + 1}">
                <li>
                    <a class="btn" href="<c:url value="/?pagina=2" />" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
            </c:if>                            
            <c:if test="${not (1 < pages + 1)}">
                <li>
                    <a class="btn disabled" href="" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
            </c:if>
        ...
        <c:if test="${not empty param.pagina}">
            <c:if test="${param.pagina < pages + 1}">
                <li>
                    <a class="btn" href="<c:url value="/?pagina=${param.pagina + 1}" />" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
            </c:if>                            
            <c:if test="${not (param.pagina < pages + 1)}">
                <li>
                    <a class="btn disabled" href="" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
